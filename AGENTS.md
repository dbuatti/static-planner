...
- **Calendar-add routing in `imessage_agent.py`** (Mini, src in `dbuatti/planner-mini`): `tick()` for `kind=="calendar"` calls `_parse_targets(body)`; **if it returns `[]` → `ask(body, mode="calendar")` (LLM conversational, via `cal_add.py`), else `parse_calendar(body)` (deterministic multi-event).** Single-line natural-language booking requests ("book / plan / schedule / reserve a ... on Sunday 6 Sep 10am") are forced to the LLM path so the exact parser can't mangle them (a live bug created a garbage "book a haircut on" event at the wrong date/time). The clean multi-line "Add these to my calendar / <title> / <dated lines>" shape stays deterministic. Parser hardening in `_parse_one_event`: won't steal a digit that's part of a time ("Sep 10am"/"Oct 7:30"), prefers an explicit-meridiem time over a bare day number, and prefers day-before-month ("7 October 2-4pm" => 7 Oct, not "October 2").
...
- **Routes**: single-line natural-language bookings ("book / plan / schedule a … on <day> <time>") → `ask(mode="calendar")` → LLM → `cal_add.py`. Clean multi-line "Add these to my calendar / <title> / <dated lines>" → deterministic `parse_calendar()`. Both call the shared `write_planner_event()`.
...
- **Verified live end-to-end**: `cal_add.py` writing an event → planner mirror ok → `git commit`(rc=0) → `git pull --rebase`(rc=0) → `git push`(rc=0) → DAY created + PARSE OK. Test events are removed afterwards.
...
- **One key ≠ two repos**: GitHub won't let a deploy key live on two repos. `planner-mini` (which also needs TOWN push for the daemon source) still needs its OWN deploy key.
...
- **Permission gotcha**: every `git reset --hard`/`git checkout <commit> -- <file>`/fresh clone regenerates `index.html`/`page.html`/`.git/index` as `-rw-r--r--`; re-apply the `g+w`+`core.sharedRepository` after any such operation.
