...
- **Time-finding request ("find me N minutes to X" / "make time for X" / "need 30 min for X")**: `DIRECTED_RE` + `FINDTIME_RE` recognize these as directed, and `classify()` routes them to the `ask` path — the LLM **proposes a concrete free slot** by reading tomorrow/this-week's schedule, without auto-writing to the planner (user confirms before any edit). Deliberately excludes bare "respond to X" (no time-finding) so stray notes aren't hijacked.
